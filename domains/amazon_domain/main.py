# Script to familiarize with the interface of AmazonDomain

from domains.amazon_domain.amazon_env import Amazon_Environment
from domains.amazon_domain.amazon_action import Action

features = {"size": ["small", "large"], "fragile": [0, 1]}
# features = {"a" : [1], "b" : [0]}
features_dict = {"state": features, "action": features}

env = Amazon_Environment(features_dict, use_existing=False)
for s in env.states:
    print(s.name)
